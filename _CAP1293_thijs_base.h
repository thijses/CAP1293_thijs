...
#ifndef _CAP1293_thijs_base_h
#define _CAP1293_thijs_base_h

#include "Arduino.h" // always import Arduino.h

#include "CAP1293_thijs.h" // (i feel like this constitutes a cicular dependency, but the compiler doesn't seem to mind)


#ifndef CAP1293_useWireLib // (note: ifNdef!) if this has been defined by the user, then don't do all this manual stuff
    ...
    #warning("using Wire library for CAP1293 (no platform optimized code available)")
    #define CAP1293_useWireLib  // use Wire as a backup
  #endif
#endif

#ifdef CAP1293_useWireLib // note: this ifdef check is done after the ifndef, so the compiler gets a chance to define it anyway
  #include <Wire.h>
...
otherwise, just a boolean, indicating general faillure should be enough, especially if you use CAP1293debugPrint() effectively
...
*/
#ifndef CAP1293_ERR_RETURN_TYPE  // unless the user already defined it manually
  #define CAP1293_ERR_RETURN_TYPE_default  bool
  #define CAP1293_ERR_RETURN_TYPE_default_OK  true
  #define CAP1293_ERR_RETURN_TYPE_default_FAIL false
  #ifdef CAP1293_useWireLib
    #define CAP1293_ERR_RETURN_TYPE  CAP1293_ERR_RETURN_TYPE_default
    #define CAP1293_ERR_RETURN_TYPE_OK  CAP1293_ERR_RETURN_TYPE_default_OK
    #define CAP1293_ERR_RETURN_TYPE_FAIL  CAP1293_ERR_RETURN_TYPE_default_FAIL
  #elif defined(ARDUINO_ARCH_ESP32) // the ESP32 likes to spit out esp_err_t for most things
    #define CAP1293_ERR_RETURN_TYPE  esp_err_t
    #define CAP1293_ERR_RETURN_TYPE_OK  ESP_OK
    #define CAP1293_ERR_RETURN_TYPE_FAIL  ESP_FAIL
    #define CAP1293_return_esp_err_t   // to let the code below know that the return type is an esp_err_t
  #elif defined(ARDUINO_ARCH_STM32)
    #define CAP1293_ERR_RETURN_TYPE  i2c_status_e
    #define CAP1293_ERR_RETURN_TYPE_OK  I2C_OK
    #define CAP1293_ERR_RETURN_TYPE_FAIL  I2C_ERROR // NOTE: this may cause some debugging confusion (but then again, STM32 I2C is already hard to debug. Just try using WireLib)
    #define CAP1293_return_i2c_status_e // to let the code below know that the return type is an esp_err_t
  #else
    #error("failed to define ERR_RETURN_TYPE for some reason")
  #endif
#endif
#ifndef CAP1293_ERR_RETURN_TYPE_OK
  #warning("CAP1293_ERR_RETURN_TYPE was manually defined, but CAP1293_ERR_RETURN_TYPE_OK was not!, attempting to infer automatically...")
  #ifdef CAP1293_return_esp_err_t
    #define CAP1293_ERR_RETURN_TYPE_OK  ESP_OK
  #elif defined(CAP1293_return_i2c_status_e)
    #define CAP1293_ERR_RETURN_TYPE_OK  I2C_OK
  #else
    #error("when manually defining the CAP1293_ERR_RETURN_TYPE, please also define the CAP1293_ERR_RETURN_TYPE_OK. See _CAP1293_thijs_base.h")
  #endif
#endif
#ifndef CAP1293_ERR_RETURN_TYPE_FAIL
  #warning("CAP1293_ERR_RETURN_TYPE was manually defined, but CAP1293_ERR_RETURN_TYPE_FAIL was not!, attempting to infer automatically...")
  #ifdef CAP1293_return_esp_err_t
    #define CAP1293_ERR_RETURN_TYPE_FAIL  ESP_FAIL
  #elif defined(CAP1293_return_i2c_status_e)
    #define CAP1293_ERR_RETURN_TYPE_FAIL  I2C_ERROR // NOTE: this may cause some debugging confusion (but then again, STM32 I2C is already hard to debug. Just try using WireLib)
  #else
    #error("when manually defining the CAP1293_ERR_RETURN_TYPE, please also define the CAP1293_ERR_RETURN_TYPE_FAIL. See _CAP1293_thijs_base.h")
  #endif
#endif
...
/**
 * (this is only the base class, users should use CAP1293_thijs)
 * 
 */
class _CAP1293_thijs_base
{
  public:
  //// I2C constants:
  static const uint8_t slaveAddress = 0x28; //7-bit address
  
  #ifdef CAP1293_useWireLib // higher level generalized (arduino wire library):
     ...
     */
    bool requestReadBytes(uint8_t registerToRead, uint8_t readBuff[], uint8_t bytesToRead=1) {
    ...
    bool onlyReadBytes(uint8_t readBuff[], uint8_t bytesToRead) {
      ...
      if(Wire.available() != bytesToRead) { CAP1293debugPrint("onlyReadBytes() received insufficient data"); return(false); }
     ...
     */
    bool writeBytes(uint8_t registerToWrite, uint8_t writeBuff[], uint8_t bytesToWrite=1) {
      Wire.beginTransmission(slaveAddress);
    ...
    */
    ...
    inline bool startWrite() {
      ...
      if(twoWireStatusReg != twi_SR_M_SLA_W_ACK) { CAP1293debugPrint("SLA_W ack error"); TWCR = twi_STOP; return(false); }
      return(true);
    }
    ...
    inline bool startRead() {
      ...
      if(twoWireStatusReg != twi_SR_M_SLA_R_ACK) { CAP1293debugPrint("SLA_R ack error"); TWCR = twi_STOP; return(false); }
      return(true);
    }
    ...
    bool requestReadBytes(uint8_t registerToRead, uint8_t readBuff[], uint8_t bytesToRead=1) {
      ...
      for(uint8_t i=0; i<(bytesToRead-1); i++) {
        ...
        twoWireTransferWait();
        //if(twoWireStatusReg != twi_SR_M_DAT_R_ACK) { CAP1293debugPrint("DAT_R Ack error"); return(false); }
        readBuff[i] = TWDR;
      }
      TWCR = twi_basic; //request 1 more byte
      twoWireTransferWait();
      //if(twoWireStatusReg != twi_SR_M_DAT_R_NACK) { CAP1293debugPrint("DAT_R Nack error"); return(false); }
    ...
    esp_err_t init(uint32_t frequency, int SDApin=21, int SCLpin=22, i2c_port_t I2CportToUse=I2C_NUM_0, gpio_pullup_t pullEnable=GPIO_PULLUP_ENABLE) {
      if(I2CportToUse < I2C_NUM_MAX) { I2Cport = I2CportToUse; } else { CAP1293debugPrint("can't init(), invalid I2Cport!"); return(ESP_ERR_INVALID_ARG); }
      i2c_config_t conf;
      ...
      if (err != ESP_OK) { CAP1293debugPrint("can't init(), i2c_param_config error!"); CAP1293debugPrint(esp_err_to_name(err)); return(err); }
    ...
    CAP1293_ERR_RETURN_TYPE requestReadBytes(uint8_t registerToRead, uint8_t readBuff[], uint8_t bytesToRead=1) {
      ...
      if(err != ESP_OK) { CAP1293debugPrint(esp_err_to_name(err)); }
      #ifdef CAP1293_return_esp_err_t
        return(err);
      #else
        return(err == ESP_OK);
...
//      esp_err_t err = i2c_master_write_to_device(I2Cport, slaveAddress, copiedArray, bytesToWrite+1, I2Ctimeout / portTICK_RATE_MS);
    ...
    void init(uint32_t frequency) {
      ...
      // the default module is all i'm going to need for my uses, but if you wanted to use multiple I2C peripherals, please uncomment all the twi_setModule() things and add a module constant to each CAP1293_thijs obj
      twi_init();
    ...
    bool requestReadBytes(uint8_t registerToRead, uint8_t readBuff[], uint8_t bytesToRead=1) {
      ...
      if(ret != 0) { CAP1293debugPrint("requestReadBytes() twi_writeTo error!"); return(false); }
      return(onlyReadBytes(readBuff, bytesToRead));
    }
    ...
    bool onlyReadBytes(uint8_t readBuff[], uint8_t bytesToRead) {
      ...
      if(readQuantity != bytesToRead) { CAP1293debugPrint("onlyReadBytes() received insufficient data"); return(false); }
      return(true);
    }
    ...
    bool writeBytes(uint8_t registerToWrite, uint8_t writeBuff[], uint8_t bytesToWrite=1) {
      ...
      if(ret != 0) { CAP1293debugPrint("writeBytes() twi_writeTo error!"); return(false); }
      return(true);
      ...
      // so, I'm just stuck copying the writeBuff to yet another buffer. Luckily, the CAP1293 only accepts 2-byte data anyway, so it's a small buffer...
    }
  ...
  /*
  the remainder of the code can be found in the main header file: CAP1293_thijs.h
  This is just a parent class, meant to hold all the low-level I2C implementations
  */
};

#endif // _CAP1293_thijs_base_h
